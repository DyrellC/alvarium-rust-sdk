use crate::{config::{self, Signable}, constants};
use crate::annotations::{Annotation, Annotator, derive_hash, sign_annotation};;
use crate::providers::sign_provider::get_priv_key;
...
fn valid_and_invalid_pki_annotator() {
    ...
    let pki_annotator_1 = PkiAnnotator::new(config.clone());
    let pki_annotator_2 = PkiAnnotator::new(config2.clone());
...
fn make_pki_annotation() {
    ...
    let pki_annotator = PkiAnnotator::new(config.clone());
